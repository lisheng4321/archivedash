...
export default function SubscriptionsPage({ ctx }) {
  ...
  const rowBtn = (extra) => ({ minHeight: isMobile ? 38 : undefined, padding: isMobile ? "9px 14px" : "4px 7px", background: "#232c3c", color: "#d1d5db", border: "none", borderRadius: 6, fontSize: isMobile ? 12 : 11, cursor: "pointer", ...extra });
    ...
    <div style={{ padding: pagePad }}>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 16, flexWrap: "wrap", gap: 8 }}>
        <div>
          ...
          <p style={{ margin: "3px 0 0", fontSize: 12, color: "#8b97ad" }}>{subStats.active.length} active · {currency(subStats.monthlyBurn)}/mo · {currency(subStats.annualCost)}/yr</p>
        </div>
      ...
      {subStats.overdue.length > 0 && (
        <div style={{ background: "#3b1f1f", border: "1px solid #ef444466", borderRadius: 12, padding: "10px 14px", marginBottom: 14, fontSize: 13, color: "#fca5a5" }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "repeat(2, 1fr)" : "repeat(4, 1fr)", gap: 10, marginBottom: 14 }}>
        ...
        <KPI label="Monthly burn" value={currency(subStats.monthlyBurn)} />
        <KPI label="Annual cost" value={currency(subStats.annualCost)} />
            ...
            return (
              <button key={category} onClick={() => setSubCatFilter(subCatFilter === category ? "All" : category)} aria-pressed={subCatFilter === category} title={subCatFilter === category ? "Clear category filter" : `Filter by ${category}`} style={{ background: subCatFilter === category ? bg : "#121a2b", border: `1px solid ${subCatFilter === category ? fg : "#232c3c"}`, borderRadius: 8, padding: "9px 12px", textAlign: "left", cursor: "pointer", fontFamily: "inherit" }}>
                <div style={{ display: "flex", justifyContent: "space-between", gap: 8, alignItems: "center" }}>
                  ...
                  <span style={{ color: "#7c8aa0", fontSize: 11 }}>{count}</span>
                </div>
                <div style={{ marginTop: 4, color: "#f3f6fb", fontSize: 14, fontWeight: 800 }}>{currency(monthly)}<span style={{ color: "#7c8aa0", fontSize: 11, fontWeight: 600 }}> /mo</span></div>
              </button>
            );
          })}
        </div>
      ...
      <div style={{ background: "#121a2b", borderRadius: 12, border: "1px solid #232c3c", overflow: "hidden" }}>
        ...
        {sortedSubs.length === 0 && (subsCount > 0 ? (
          <div style={{ padding: 36, textAlign: "center", color: "#8b97ad", fontSize: 13 }}>
