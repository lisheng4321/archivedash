          ...
          {rows.map((comp) => (
            <div key={comp.id} style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : onHide ? "1fr 86px 78px 58px" : "1fr 86px 78px", gap: isMobile ? 5 : 10, padding: "10px 14px", borderBottom: "1px solid #232c3c11", alignItems: "center" }}>
              <div style={{ minWidth: 0 }}>
                ...
                <div style={{ color: "#8b97ad", fontSize: 11, marginTop: 3 }}>{compSubtitle(comp)}</div>
              </div>
              <div style={{ color: "#f3f6fb", fontSize: 12, fontWeight: 800, textAlign: "right" }}>
                {currency(comp.total)}
                {comp.couponPrice !== undefined && <div style={{ color: "#60a5fa", fontSize: 11, fontWeight: 700 }}>coupon</div>}
              </div>
              <div style={{ color: "#9ca3af", fontSize: 11 }}>{comp.scope.toUpperCase()}</div>
              {onHide && <button onClick={() => onHide(comp)} style={{ ...ghostBtn, padding: "4px 7px", fontSize: 11 }}>Hide</button>}
            </div>
          ))}
        </div>
      )}
          ...
          <div key={comp.id} style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : onInclude ? "1fr 150px 86px 66px" : "1fr 150px 86px", gap: isMobile ? 5 : 10, padding: "10px 14px", borderBottom: "1px solid #232c3c11", alignItems: "center" }}>
            ...
            <div style={{ display: "flex", gap: 4, flexWrap: "wrap" }}>
              {comp.reasons.map((reason) => (
                <span key={reason} style={{ padding: "2px 6px", borderRadius: 6, background: "#3b1f1f", color: "#fca5a5", fontSize: 11, fontWeight: 800 }}>{reason}</span>
              ))}
            </div>
            <div style={{ color: "#7c8aa0", fontSize: 12, fontWeight: 700, textAlign: "right" }}>{currency(comp.total)}</div>
            {onInclude && (comp.ownSellerExcluded || comp.isOwnListing
              ? <span style={{ color: "#93c5fd", fontSize: 11, fontWeight: 800 }}>Mine</span>
              : <button onClick={() => onInclude(comp)} style={{ ...ghostBtn, padding: "4px 7px", fontSize: 11, color: "#86efac" }}>Include</button>)}
          </div>
        ))
      )}
    </div>
  ...
  return (
    <div style={{ ...panel, padding: 14 }}>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "repeat(4, minmax(0, 1fr))", gap: 8 }}>
        {points.map((point) => (
          <div key={point.label} style={{ background: "#0d1117", border: "1px solid #232c3c", borderRadius: 8, padding: 10, minWidth: 0 }}>
            <div style={{ color: "#7c8aa0", fontSize: 11, fontWeight: 800, textTransform: "uppercase" }}>{point.label}</div>
            ...
            <div style={{ color: "#8b97ad", fontSize: 11, marginTop: 2 }}>{point.margin.toFixed(1)}% margin</div>
          </div>
        ))}
      </div>
    </div>
      ...
      <div style={{ display: "grid", gridTemplateColumns: isMobile ? "1fr" : "320px minmax(0, 1fr)", gap: 14, alignItems: "start" }}>
        <div style={{ display: "flex", flexDirection: "column", gap: 10, position: isMobile ? "static" : "sticky", top: 12, maxHeight: isMobile ? "none" : "calc(100vh - 190px)", minHeight: 0 }}>
          <div style={{ ...panel, padding: 10, display: "grid", gridTemplateColumns: "1fr", gap: 8, flexShrink: 0 }}>
            ...
            <div style={{ color: "#8b97ad", fontSize: 11, fontWeight: 700 }}>{visibleReviews.length} shown</div>
          </div>
          ...
          {visibleReviews.length === 0 && <div style={{ ...panel, padding: 16, color: "#8b97ad", fontSize: 12, textAlign: "center" }}>No products match these filters.<button onClick={() => { setCardSearch(""); setCardFilter("all"); setCardSort("recommended"); }} style={{ ...ghostBtn, display: "block", margin: "10px auto 0", padding: "5px 12px", fontSize: 11 }}>Clear filters</button></div>}
          </div>
        </div>
          ...
          {showTuning && (
            <div style={{ ...panel, padding: 14 }}>
              ...
              <div style={{ display: "flex", gap: 8, flexWrap: "wrap", marginTop: 12 }}>
                ...
                <span style={{ color: "#8b97ad", fontSize: 11, alignSelf: "center" }}>Run sync again after changing the query.</span>
              </div>
            </div>
          )}
