...
import { draftChanged, recordError } from "../formValidation.js";
...
function EditExpModal({ expense, onSave, onClose, paymentMethods = [] }) {
  ...
  const gc = () => { if (saving) return; if (draftChanged(ef, initial.current, ["amount"])) setShowU(true); else onClose(); };
  const submit = async () => {
    const issue = recordError(ef.name, ef.amount, "Amount"); setError(issue); if (issue || saving) return;
    setSaving(true);
    try { const result = await onSave({ ...expense, ...ef, name: ef.name.trim(), amount: Number(ef.amount) }); if (result?.ok === false) setError(result.error || "Could not save. Try Save again."); }
    finally { setSaving(false); }
  };
  return (<><Modal open={true} onClose={onClose} guardedClose={gc} title="Edit expense" dismissible={!saving}>
