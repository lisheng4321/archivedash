  ...
  const invRow = (item, isGroupChild, index = 0) => {
    ...
    return (
      <div key={item.id} onClick={(e) => rowClick(e, toggleSel, item.id)} style={{ display: "grid", gridTemplateColumns: inventoryGridColumns, gap: 5, padding: isGroupChild ? "8px 16px 8px 46px" : "10px 16px", alignItems: "center", fontSize: 13, borderBottom: "1px solid #232c3c11", background: rowBg(index, selectedInv.has(item.id)), cursor: "pointer", ...(isGroupChild ? childAccent : {}) }}>
        ...
        <span style={{ color: "#7c8aa0", fontSize: 11, textAlign: "left" }}>{item.purchaseDate}</span>
        <span style={{ color: "#7c8aa0", fontSize: 11, textAlign: "right" }}>1</span>
        <div style={{ display: "flex", gap: 3, justifyContent: "center" }}>
          <button onClick={() => setSellOpen(item)} style={{ padding: "4px 7px", background: "#2563eb", color: "#fff", border: "none", borderRadius: 6, fontSize: 11, cursor: "pointer", fontWeight: 500 }}>Sell</button>
          <button onClick={() => setEditInvOpen(item)} style={{ padding: "4px 7px", background: "#232c3c", color: "#d1d5db", border: "none", borderRadius: 6, fontSize: 11, cursor: "pointer" }}>Edit</button>
          <button onClick={() => duplicateItem(item)} title="Duplicate" style={{ padding: "4px 7px", background: "#232c3c", color: "#a78bfa", border: "none", borderRadius: 6, fontSize: 11, cursor: "pointer" }}>⧉</button>
          <button onClick={() => setConfirmDel({ type: "inv", id: item.id, name: item.name })} style={{ padding: "4px 7px", background: "#232c3c", color: "#f87171", border: "none", borderRadius: 6, fontSize: 11, cursor: "pointer" }}>✕</button>
        </div>
      </div>
    );
  };
  ...
  const groupRow = (item, isExpanded, key, index = 0) => {
    ...
    if (isMobile) {
      return (
        <div onClick={() => toggleGroup(key)} style={{ padding: "10px 12px", display: "flex", alignItems: "center", gap: 10, cursor: "pointer", background: rowBg(index, false), borderBottom: "1px solid #232c3c22", ...groupAccent }}>
          <div onClick={(e) => { e.stopPropagation(); toggleGroupSelection(item._items || []); }} style={{ width: 44, height: 44, margin: "-9px 0 -9px -10px", display: "flex", alignItems: "center", justifyContent: "center", flexShrink: 0 }}><input type="checkbox" checked={groupChecked} onChange={(e) => { e.stopPropagation(); toggleGroupSelection(item._items || []); }} onClick={(e) => e.stopPropagation()} style={{ ...cb, width: 20, height: 20 }} /></div>
    ...
    return (
      <div onClick={() => toggleGroup(key)} style={{ display: "grid", gridTemplateColumns: inventoryGridColumns, gap: 5, padding: "10px 16px", alignItems: "center", fontSize: 13, borderBottom: "1px solid #232c3c22", cursor: "pointer", background: rowBg(index, false), ...groupAccent }}>
        ...
        <span style={{ color: "#7c8aa0", fontSize: 11, textAlign: "left" }}>{groupDateLabel(item._items || [])}</span>
    ...
    return (
      <div key={s.id} onClick={(e) => rowClick(e, toggleSelSale, s.id)} style={{ display: "grid", gridTemplateColumns: salesGridColumns, gap: 4, padding: "10px 16px", alignItems: "center", fontSize: 13, borderBottom: "1px solid #232c3c11", background: rowBg(index, selectedSales.has(s.id)), cursor: "pointer" }}>
        ...
        <div style={{ display: "flex", gap: 4, justifyContent: "center" }}>
          <button onClick={() => setEditSaleOpen(s)} style={{ padding: "3px 7px", background: "#232c3c", color: "#d1d5db", border: "none", borderRadius: 6, fontSize: 11, cursor: "pointer" }}>Edit</button>
          <button onClick={() => setConfirmDel({ type: "sale", id: s.id, name: s.name })} style={{ padding: "3px 7px", background: "#232c3c", color: "#f87171", border: "none", borderRadius: 6, fontSize: 11, cursor: "pointer" }}>✕</button>
        </div>
      </div>
    );
  };
    ...
    return (
      <div key={e.id} onClick={(ev) => rowClick(ev, toggleSelExp, e.id)} style={{ display: "grid", gridTemplateColumns: "48px 2fr 1.2fr 90px 100px 80px", gap: 6, padding: "11px 16px", alignItems: "center", fontSize: 13, borderBottom: "1px solid #232c3c11", background: rowBg(index, selectedExp.has(e.id)), cursor: "pointer" }}>
        ...
        <div style={{ display: "flex", gap: 4 }}>
          <button onClick={() => setEditExpOpen(e)} style={{ padding: "3px 7px", background: "#232c3c", color: "#d1d5db", border: "none", borderRadius: 6, fontSize: 11, cursor: "pointer" }}>Edit</button>
          <button onClick={() => setConfirmDel({ type: "exp", id: e.id, name: e.name })} style={{ padding: "3px 7px", background: "#232c3c", color: "#f87171", border: "none", borderRadius: 6, fontSize: 11, cursor: "pointer" }}>✕</button>
        </div>
      </div>
    );
  };
